 ...
 *  - PrismaClient: makePrismaMock({ 'user.findFirst': userRow, ... })
...
function makeService(opts: { user?: any | null } = {}) {
  const userRow = opts.user === undefined ? ACTIVE_USER : opts.user;
  const prisma = makePrismaMock({
    'user.findFirst': userRow,
...
describe('AuthService.login', () => {
  ...
  it('用户名不存在 → 401', async () => {
    ...
    await expect(service.login(TEST_TENANT_ID, 'nobody', TEST_PASSWORD)).rejects.toThrow(
  ...
  it('密码错误 → 401(不会暴露用户名是否存在)', async () => {
    const { service } = makeService();
    await expect(service.login(TEST_TENANT_ID, 'admin', 'wrong-password')).rejects.toThrow(
      UnauthorizedException,
    );
  });
  ...
  it('已禁用用户(userStatus !== ACTIVE)被 prisma 过滤后等同于"用户不存在"', async () => {
    ...
    await expect(service.login(TEST_TENANT_ID, 'admin', TEST_PASSWORD)).rejects.toThrow(
      UnauthorizedException,
    );
  ...
  it('用户属于别的租户 → 同样 401', async () => {
    ...
    const { service } = makeService({ user: null });
    await expect(service.login('tenant-other', 'admin', TEST_PASSWORD)).rejects.toThrow(
      UnauthorizedException,
    );
  });
});
